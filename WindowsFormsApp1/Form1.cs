...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private const string NoNodeTxet = "empty";
        ...
        public static Icon GetFolderIcon(string folderName, bool isSmallIcon)
        {
            long imageIndex = ErrorFolderIndex;
        ...
        {
            string[] slist = Directory.GetLogicalDrives();
            for (int i = 0; i < slist.Length; i++)
            {
                var tvn = new TreeNode(slist[i], new TreeNode[] { new TreeNode(NoNodeTxet) });
                tvn.Tag = slist[i];
                if(!imgListFlies.Images.ContainsKey(slist[i]))
                {
                    imgListFlies.Images.Add(slist[i], GetDriverIcon(slist[i], true));
                }
                tvn.ImageKey = slist[i];
                tvn.SelectedImageKey = slist[i];
                tvPath.Nodes.Add(tvn);
            }
        }
        
        private void tvPath_BeforeExpand(object sender, TreeViewCancelEventArgs e)
        {
            if (e.Node.Nodes.Count == 1)
            {
                var tntmp = e.Node.Nodes[0];
                if (tntmp.Tag == null)
                {
                    e.Node.Nodes.Clear();
                    var stmp = e.Node.FullPath.Replace("\\\\", "\\");
                    try
                    {
                        var sImgKey = string.Empty;
                        //获取目录列表信息
                        var sPaths = Directory.GetDirectories(stmp);
                        for (int i = 0; i < sPaths.Length; i++)
                        {
                            var spath = sPaths[i].Replace(stmp, "").Trim('\\');
                            var tvn = new TreeNode(spath, new TreeNode[] { new TreeNode(NoNodeTxet) });
                            tvn.Tag = sPaths[i];

                            sImgKey = "Directory";
                            if (!imgListFlies.Images.ContainsKey(sImgKey))
                            {
                                imgListFlies.Images.Add(sImgKey, GetFolderIcon(sPaths[i], true));
                            }
                            tvn.ImageKey = sImgKey;
                            tvn.SelectedImageKey = sImgKey;
                            e.Node.Nodes.Add(tvn);
                        }
                        //获取文件列表信息
                        sPaths = Directory.GetFiles(stmp);
                        for (int i = 0; i < sPaths.Length; i++)
                        {
                            var spath = sPaths[i].Replace(stmp, "").Trim('\\');
                            var tvn = new TreeNode(spath);
                            tvn.Tag = sPaths[i];

                            var sExtension = Path.GetExtension(sPaths[i]).ToLower();
                            sImgKey = sExtension;
                            if (sExtension == ".exe")
                            {
                                sImgKey = sPaths[i];
                            ...
                            if (!imgListFlies.Images.ContainsKey(sImgKey))
                            {
                                imgListFlies.Images.Add(sImgKey, Icon.ExtractAssociatedIcon(sPaths[i]));
                            }
                            tvn.ImageKey = sImgKey;
                            tvn.SelectedImageKey = sImgKey;
                            e.Node.Nodes.Add(tvn);
